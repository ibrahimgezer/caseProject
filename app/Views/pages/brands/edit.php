...
<?= $this->section('content') ?>
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark">Marka Düzenle</h1>
                </div>
                <div class="col-sm-6">
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <?php if (!empty(session()->getFlashdata('brand_edit_failed'))) : ?>
                        <div class="alert alert-danger my-2 small">
                            <?= session()->getFlashdata('brand_edit_failed'); ?>
                        </div>
                    <?php
                    endif;
                    if (!empty(session()->getFlashdata('brand_edit_success'))) :
                        ?>
                        <div class="alert alert-success my-2 small">
                            <?= session()->getFlashdata('brand_edit_success'); ?>
                        </div>
                        <script>
                            setTimeout(function () {
                                location.reload();
                    ...
                    <div class="card card-info">
                        <?php if (isset($brandData)) : ?>
                            <form class="form-horizontal" action="<?= base_url("update_brand/" . $brandData["id"]) ?>"
                                  method="post">
                                <div class="card-body">
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-10">
                                            <input type="text" class="form-control" id="name" name="name"
                                                   value="<?= isset($brandData) ? $brandData["name"] : "" ?>"
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <textarea id="description" name="description" class="form-control" rows="3"
                                                  placeholder="Açıklama"><?= isset($brandData) ? $brandData["description"] : "" ?></textarea>
                                                ...
                                                <?php if (!empty($categories) && count($categories) > 0) { ?>
                                                    <?php foreach ($categories as $category) { ?>
                                                        <option value="<?= $category["id"] ?>"
                                                            <?= isset($brandData) && $brandData["category_id"] == $category["id"] ? "selected" : "" ?> >
